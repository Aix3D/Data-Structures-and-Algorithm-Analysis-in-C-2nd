...
int eulerTour (Adjacenty_List * const padj, const Hash_Table * const pht, const int start) ;
...
void buildGraph (Adjacenty_List * const padj, Hash_Table * const pht)
{
	...
	InitializeALine_A (padj, pht, 1, 'b', 1, 2, 'c', 0) ;
	...
	InitializeALine_A (padj, pht, 7, 'h', 1, 2, 'i', 0) ;
...
int eulerTour (Adjacenty_List * const padj, const Hash_Table * const pht, const int start)
{
	...
	Rqueue_Node * scan ;
	int capacity = (*padj) -> capacity, i, count, may_start_1st = INFINITY, may_start_2nd = INFINITY ;
	...
	for (i = 0, count = 0; i < capacity; i++)
	{
		...
		{
			if (INFINITY == may_start_1st)
				may_start_1st = i ;
			else if (INFINITY == may_start_2nd)
				may_start_2nd = i ;
			count++ ;
		}
	}
	if (count > 2 || 1 == count)
		return 0 ;
	if (2 == count && start != may_start_1st && start != may_start_2nd)
		return 0 ;
